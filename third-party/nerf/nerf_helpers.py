...
def joint_positional_encoding(
    ...
    frequency_bands = torch.cat([torch.zeros(1).cuda(), frequency_bands[:-1]], 0)

    # convert to spherical coordinate
    tensor_u = torch.atan2(tensor[:,0], tensor[:,2]) # -pi, pi
    tensor_v = torch.asin(tensor[:,1])  # -pi/2, pi/2
    for frequ in frequency_bands:
        for freqv in frequency_bands:
           for func in [torch.sin, torch.cos]:
               encoding.append(func(tensor_u[:,None] * frequ + 
                                    tensor_v[:,None] * freqv    ))
#    for freqx in frequency_bands:
#        for freqy in frequency_bands:
#            for freqz in frequency_bands:
#                for func in [torch.sin, torch.cos]:
#                    encoding.append(func(tensor[:,0:1] * freqx + 
#                                         tensor[:,1:2] * freqy +
#                                         tensor[:,2:3] * freqz    ))
...
def get_joint_embedding_function(
    num_encoding_functions=6, include_input=True, log_sampling=True
):
    r"""Returns a lambda function that internally calls positional_encoding.
    """
    return lambda x: joint_positional_encoding(
        x, num_encoding_functions, include_input, log_sampling
    )

def time_positional_encoding(
    tensor, num_encoding_functions=6, include_input=True, log_sampling=True
) -> torch.Tensor:
    r"""Apply positional encoding to the input.
...
def get_time_embedding_function(
    num_encoding_functions=6, include_input=True, log_sampling=True
):
    r"""Returns a lambda function that internally calls positional_encoding.
    """
    return lambda x: time_positional_encoding(
        x, num_encoding_functions, include_input, log_sampling
    )
